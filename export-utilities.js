...
// Export functions for the main interface
function exportToPDF() {
    if (!window.currentSATBData) {
        alert('No SATB data available to export. Please generate SATB harmonization first.');
        return;
    }
    ...
    const htmlContent = `
        ...
        <html>
        <head>
            <title>SATB Harmonization - ${window.currentSATBData.progression || 'Generated'}</title>
            <style>
        ...
        <body>
            <div class="header">
                <h1>SATB Harmonization</h1>
                <p>Generated on ${new Date().toLocaleDateString()}</p>
            </div>
            <div class="info">
                <p><strong>Progression:</strong> ${window.currentSATBData.progression || 'N/A'}</p>
                <p><strong>Key:</strong> ${window.currentSATBData.key || 'N/A'}</p>
            </div>
            <div class="score">
                <h3>SATB Notation</h3>
    ...
    // Create and download the PDF-like HTML file
    const blob = new Blob([htmlContent], { type: 'text/html' });
    const url = window.URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'satb-harmonization.html';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    window.URL.revokeObjectURL(url);
    
    alert('SATB harmonization exported as HTML file. Open it in your browser and use Print (Ctrl+P) to save as PDF.');
}
...
function exportToMusicXML() {
    ...
    try {
        const musicXML = window.satbExportManager.exportMusicXML(
            window.currentSATBData.satb,
            window.currentSATBData.rhythm || ['q', 'q', 'q', 'q'],
            window.currentSATBData.progression || ['I', 'IV', 'V', 'I'],
            {
                title: 'SATB Harmonization',
                composer: 'Generated by SATB Writing Website',
                key: window.currentSATBData.key || 'C major'
            }
        );
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = 'satb-harmonization.musicxml';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        window.URL.revokeObjectURL(url);
        
        alert('SATB harmonization exported as MusicXML file successfully!');
    } catch (error) {
        console.error('Error exporting MusicXML:', error);
        alert('Error exporting MusicXML file. Please try again.');
    }
} 